...
from pyecharts.charts import Page, Line, Grid, Tab, Bar
...
class BaseDataSet(object):
    ...
    @staticmethod
    def get_tradingday_range(trade_calendar_path: str, start_date: int, end_date: int, timeWindow: int = 1, delay: int = 0) -> List[int] :
    ...
    @staticmethod
    def read_daily_data(data_path: str, date_range: List[int], yesterday_range: List[int], file_name: str, variable_name: str, ticker_list = None, time: int = 145900) -> xr.DataArray:
    ...
    @staticmethod
    def read_minute_data(data_path: str, date_range: List[int] = None, ticker_list: List[int] = None, fields: List[str] = None, timelist: List[int] = None) -> xr.DataArray:
        Data = None
    ...
    def select_date(self, Return_path: str = None):
        ...
        if Return_path is None:
            if ('Return_path' in setting.keys()) and (setting['Return_path'] is not None):
                Return_path = setting['Return_path']
            else:
        ...
        if read_st_ed != []:
            read_range = BaseDataSet.get_tradingday_range(trade_calendar_path=setting['trade_calendar_path'], start_date=read_st_ed[0], end_date=read_st_ed[1], timeWindow=1, delay=0)
        else:
            read_range = []
    ...
    @staticmethod
    def prepare_market(market_path: str, date_range: List[int], ticker_list: List[int], fields, trade_calendar_path: str, market_processed_path: str = None, timelist: List[int] = None) -> xr.DataArray:
        ...
        market_data = BaseDataSet.read_minute_data(data_path=market_path,date_range=date_range,ticker_list=ticker_list,fields=fields,timelist=timelist)
        # 用新的ticker_list读前一天的数据
        # data_path0是上一段数据的存储路径
        previous_date = BaseDataSet.get_tradingday_range(trade_calendar_path, start_date = date_range[0], end_date = date_range[0], delay = 1)
        ...
        if os.path.exists(os.path.join(market_processed_path,f'{previous_date[0]}.h5')):
            market_data_previous = BaseDataSet.read_minute_data(data_path=market_processed_path,date_range=previous_date,ticker_list=ticker_list,fields=fields,timelist=timelist)
        else:
            market_data_previous = BaseDataSet.read_minute_data(data_path=market_path,date_range=previous_date,ticker_list=ticker_list,fields=fields,timelist=timelist)
        market_data = xr.concat([market_data,market_data_previous],dim = 'date')
        market_data = market_data.sortby('date',ascending= True)
        # 再填充
        # ClosePrice和PreDayClose为0的值替换为np.nan,然后ClosePrice按datetime从前往后填,用前一天145900的Close填充第二天92500的PreDayClose
        ...
        # preDayClose全部重新赋值?
        # preDayClose,_ = xr.broadcast(market_data.loc[:,:,145900,'preDayClose'].shift(date = 1),market_data.loc[:,:,:,'preDayClose'])
        # market_data.loc[:,:,:,'preDayClose'] = preDayClose
        market_data.loc[:,:,92500,'preDayClose'] = market_data.loc[:,:,:,'preDayClose'].stack(datetime = ('date','time')).fillna(market_data.loc[:,:,:,'Close'].stack(datetime = ('date','time')).shift(datetime=1)).unstack().loc[:,:,92500]
    ...
    @staticmethod
    def prepare_return(market_data: xr.DataArray, timelist: List[int] = None) -> xr.DataArray:
        # timelist 每天固定的几个时间点,计算这些时间点之间的Return.
        # 可以在后续改变时间点时重新调用计算Return
        Return = xr.zeros_like(market_data)
        Return['variable'] = ['MinuteReturn','AccumulatedReturn']
        if timelist is None:
            timelist = market_data['time'].values
        ...
        Return.loc[:,:,:,'AccumulatedReturn'] = market_data.loc[:, :, :, 'Close'] / market_data.loc[:, :, market_data['time'].values[0], 'preDayClose']
        return Return
    ...
    @staticmethod
    def save_DataArray(DataArrayDict: dict, save_path, byDate: bool = True, separateFolder: bool = False, datelist: List[int] = None):  
    ...
    def load_cache(self, read_range, varlist_read: str, read_path: str = None):
        ...
        if os.path.exists(read_path):
            for var in varlist_read:
                self.__dict__[var] = BaseDataSet.concat_on_dim(self.__dict__[var],BaseDataSet.read_minute_data(data_path=os.path.join(read_path,var),date_range=read_range,ticker_list=self.ticker_list,timelist=self.timelist),dim = 'date').sortby('date',ascending= True)
    ...
    def prepare_data(self, setting: dict):
        ...
        # 获取时间范围
        # 根据起止日期，获取交易日列表
        self.next_date = BaseDataSet.get_tradingday_range(trade_calendar_path=setting['trade_calendar_path'], start_date=self.end_date, end_date=self.end_date, delay=-1)[0]
        self.date_range = BaseDataSet.get_tradingday_range(trade_calendar_path=setting['trade_calendar_path'], start_date=self.start_date, end_date=self.next_date, timeWindow=1, delay=0)
        if self.daily_lag:
            # 由于日频数据需要滞后一天处理，获取前一个交易日列表
            self.yesterday_range = list(int(yesterday) for yesterday in BaseDataSet.get_tradingday_range(trade_calendar_path=setting['trade_calendar_path'],start_date=self.start_date, end_date=self.next_date, timeWindow=1, delay=1))
        else:
            self.yesterday_range = None
        # 获取股票范围
        # 先读取univ flag，并据此获取ticker_list
        self.univ_flag = BaseDataSet.read_daily_data(data_path=setting['daily_data_path'], date_range=self.date_range, yesterday_range=self.yesterday_range, file_name=setting['univ_flag_file_name'], variable_name='univ_flag', ticker_list=None).fillna(0)
        ...
        if remain_range != []:
            ...
            if self.daily_lag:
                # 由于日频数据需要滞后一天处理，获取前一个交易日列表
                remain_yesterday_range = list(int(yesterday) for yesterday in BaseDataSet.get_tradingday_range(trade_calendar_path=setting['trade_calendar_path'],start_date=remain_range[0], end_date=remain_range[-1], timeWindow=1, delay=1))
            else:
                remain_yesterday_range = None
            if 'market' in varlist_prepare:
                remainVar.update(market = BaseDataSet.prepare_market(market_path=setting['market_path'],date_range=remain_range,ticker_list=self.ticker_list,fields=setting['market_fields'], trade_calendar_path=setting['trade_calendar_path'],timelist=self.timelist))
                if 'Return' in varlist_prepare:
                    remainVar.update(Return = BaseDataSet.prepare_return(market_data=remainVar['market'],timelist=setting['timelist']))
            # 从h5文件中，读取需要的分钟线交易数据
            if 'transaction' in varlist_prepare:
                remainVar.update(transaction = BaseDataSet.read_minute_data(data_path=setting['transaction_path'], date_range=remain_range,ticker_list=self.ticker_list,fields=setting['transaction_fields'],timelist=self.timelist))
            # 从h5文件中，读取需要的分钟线index数据
            if 'index' in varlist_prepare:
                remainVar.update(index = BaseDataSet.read_minute_data(data_path=setting['index_path'], date_range=remain_range,ticker_list=self.ticker_list,fields=setting['index_fields'],timelist=self.timelist))
            # 读取风险因子并进行格式处理（日频）                   #路径可能单独存放
            if 'risk_factor' in varlist_prepare:
                remainVar.update(risk_factor = BaseDataSet.prepare_style(data_path=setting['risk_factor_path'], riskList=setting['risk_list'], date_range=remain_range, yesterday_range=remain_yesterday_range,ticker_list=self.ticker_list))
            # 读取股票行业哑变量并进行格式处理（之后会转为日频，目前为单一值）
            if 'industry_dummies' in varlist_prepare:
                remainVar.update(industry_dummies = BaseDataSet.prepare_industry_dummies(data_path=setting['daily_data_path'], date_range=remain_range,ticker_list=self.ticker_list))
            # 读取股票行业并进行格式处理（之后会转为日频，目前为单一值
            if 'industry' in varlist_prepare:
                remainVar.update(industry = BaseDataSet.prepare_industry(data_path=setting['daily_data_path'], date_range=remain_range,ticker_list=self.ticker_list))
            # 读取股票市值并进行格式处理（日频
            if 'capital' in varlist_prepare:
                remainVar.update(capital = BaseDataSet.read_daily_data(data_path=setting['daily_data_path'], date_range=remain_range, yesterday_range=remain_yesterday_range, file_name='weight.csv', variable_name='capital',ticker_list=self.ticker_list))
            # 读取benchmark并进行格式处理（日频
            if 'benchmark' in varlist_prepare:
                remainVar.update(benchmark = BaseDataSet.read_daily_data(data_path=setting['daily_data_path'], date_range=remain_range, yesterday_range=remain_yesterday_range, file_name='W_bm_zz_500_close.csv', variable_name='benchmark',ticker_list=self.ticker_list, time = 92500))
            ...
            if ('flag' in varlist_prepare) or ('price_ipo_st_flag' in varlist_prepare):
                ...
                if 'ipo_st_flag' in setting['flag_list']:
                    ipo_st_flag = BaseDataSet.read_daily_data(data_path=setting['daily_data_path'], date_range=remain_range, yesterday_range=remain_yesterday_range, file_name=setting['ipo_st_flag_file_name'], variable_name='ipo_st_flag',ticker_list=self.ticker_list).fillna(0)
    ...
    def calc_y(self, lag: int = None, y_name: str = 'y', save_path: str = None):
        """calc_y 说明
        ...
        if save_path is None:
            if (y_name+'_path' in setting.keys()) and (setting[y_name+'_path'] is not None):
                save_path = setting[y_name+'_path']
            else:
                save_path = os.path.join(setting['dst_dir'],'BaseDataSet')
        if lag is None:
        ...
        # high
        # pre_return.loc[:,:,:,'high'] = mkd.shift(date=-1).dropna(dim ='date', how ='all').isel(time=mkd.shift(date=-1).dropna(dim='date',how='all').fillna(0).argmax(dim ='time', skipna=True))/mkd-1
        pre_return.loc[:,:,:,'high'] = mkd.shift(date=-1).dropna(dim ='date', how ='all').max(dim ='time', skipna=True)/mkd-1
        # low_next
        # low.loc[:,:,:,'low_next'] = mkd.shift(date=-1).dropna(dim ='date', how ='all').isel(time=mkd.shift(date=-1).dropna(dim='date',how='all').fillna(0).argmin(dim ='time', skipna=True))/mkd-1
        low.loc[:,:,:,'low_next'] = mkd.shift(date=-1).dropna(dim ='date', how ='all').min(dim ='time', skipna=True)/mkd-1
        # low_today
        for itime in pre_return['time']:
            # low.loc[:,:,itime,'low_today'] = (mkd.loc[:,:,itime:].isel(time = mkd.loc[:,:,itime:].fillna(0).argmin(dim ='time', skipna=True))/mkd.sel(time=itime)-1)[:,:-1]
        ...
        # 保存数据
        self.__dict__[y_name] = pre_return
        BaseDataSet.save_DataArray({y_name:self.__dict__[y_name]}, save_path, byDate=True, separateFolder = True)
        print('Saved here%s'%save_path)
        
    def add_y(self, y: xr.DataArray = None, y_name: str = 'y', y_path: str = None, dim: str = None):
        """add_y 说明:
        ...
        if y is not None:
            ...
            if y_path is None:
                if ('y_path' in setting.keys()) and (setting['y_path'] is not None):
                    y_path = setting['y_path']
                else:
                    y_path = os.path.join(setting['dst_dir'],'BaseDataSet',y_name)
            if os.path.exists(y_path):
                if dim is not None:
                    self.__dict__[y_name] = BaseDataSet.concat_on_dim(self.__dict__[y_name],BaseDataSet.read_minute_data(y_path,date_range=self.date_range,ticker_list=self.ticker_list),dim = dim)
                else:
                    self.__dict__[y_name] = BaseDataSet.read_minute_data(y_path,date_range=self.date_range,ticker_list=self.ticker_list)
            else:
                print('该目录不存在')

    def export_variables(self):
        variables_map = {k: v['variable'].values.tolist()
...
class BaseAlphaSet(object):
    ...
    def __prepare(self,setting):
        self.date_range = BaseDataSet.get_tradingday_range(trade_calendar_path=setting['trade_calendar_path'], start_date=self.start_date, end_date=self.end_date, timeWindow=1, delay=0)
        univ_flag = BaseDataSet.read_daily_data(data_path=setting['daily_data_path'], date_range=self.date_range, yesterday_range=None, file_name=setting['univ_flag_file_name'], variable_name='univ_flag', ticker_list=None).fillna(0)
    ...
    def add_alpha(self, alpha_dir: str = None, alpha_names: List[str] = None, alphas_array: xr.DataArray=None,
            residual_dir: dict=None, residual_names: List[str] = None, residuals_array: dict=None):
        ...
        if residual_dir is not None:
            for iFitter_name,iResidual_dir in residual_dir.items():
                iResiduals_array = BaseDataSet.read_minute_data(data_path=iResidual_dir,date_range=self.date_range,ticker_list=self.ticker_list,fields=residual_names,timelist=self.timelist)
                self.residual.update({iFitter_name:iResiduals_array})
        if alphas_array is not None:
            self.alpha = concat_on_dim(self.alpha, alphas_array, dim='variable')
            self.alpha_names.extend(list(alphas_array['variable'].values))
        if alpha_dir is not None:
            alphas_array = BaseDataSet.read_minute_data(data_path=alpha_dir,date_range=self.date_range,ticker_list=self.ticker_list,fields=alpha_names,timelist=self.timelist)          
            self.alpha = concat_on_dim(self.alpha, alphas_array, dim='variable')
            self.alpha_names.extend(list(alphas_array['variable'].values))
    ...
    def calculate(self, dataset, handle_data, timewindow: int=1, data_require: dict={}, add=False, **kwargs):
        data = xr.concat([
             getattr(dataset, member).sel(variable=field) for member, field in data_require.items()
        ], dim='variable')
        ...
        def __handle_data(date):
            ...
            datelist = BaseDataSet.get_tradingday_range(trade_calendar_path=setting['trade_calendar_path'],
                                                        start_date=date, end_date=date, timeWindow=timewindow)
...
class LinearRegressionFitter(BaseFitter):
    ...
    def OLS(Y, X, W=None, onlyResid=True, check_singular=True, get_rsquared=False):
        ...
        W = W[:,np.newaxis] # type: np.ndarray
        if W is None:
            W = np.ones(Y.shape)
            wX, wY = X, Y
        else:
        ...
        if not onlyResid:
            bse = np.expand_dims(
                np.sqrt(
                    np.diag(
                        invFctCov * (wY - wX.dot(beta)).T.dot(wY) / 
                        ((W[:, 0] != 0).sum() - flag.sum())
                    )
                ), 
                axis=1
    ...
    def pre_processing(self, alphaset: BaseAlphaSet,
                       dataset: BaseDataSet,
                       save_path: str = None,
                       orth: bool = True,
                       method: str = 'indmean',
        ...
        def select_date(alphaset: BaseAlphaSet, resid_path: str):
            # 默认Return_path里面的日期是连续的
            read_st_ed = []
            if os.path.exists(resid_path):
                existfactor = os.listdir(resid_path)
                if all([factor in existfactor for factor in alphaset.alpha_names]):
                    start_all, end_all = [], []
                    for factor in alphaset.alpha_names:
                        file_list = sorted(os.listdir(os.path.join(resid_path,factor)))
                        if file_list != []:
                            start, end = int(file_list[0].split('.')[0]), int(file_list[-1].split('.')[0])
                            start_all.append(start)
                            end_all.append(end)
                    if start_all != []:
                        start, end = max(start_all), min(end_all)
                        if (start <= alphaset.start_date) & (end >= alphaset.end_date):
                            read_st_ed = [alphaset.start_date,alphaset.end_date]
                        elif (start <= alphaset.start_date) & (end < alphaset.end_date):
                            read_st_ed = [alphaset.start_date,end]
                        elif (start > alphaset.start_date) & (end >= alphaset.end_date):
                            read_st_ed = [start,alphaset.end_date]
                else:
                    print('某些因子尚未处理过')
            ...
            if read_st_ed != []:
                ...
                read_range = []
            remain_range = list(pd.Series(alphaset.date_range).loc[~pd.Series(alphaset.date_range).isin(pd.Series(read_range))])
            return read_range, remain_range

        def load_cache(alphaset: BaseAlphaSet, read_range: List[int], read_path: str):
            # !list
            for factor in alphaset.alpha_names:
                alphaset.residual[self.fitter_name] = BaseDataSet.concat_on_dim(
                    alphaset.residual[self.fitter_name],
                    BaseDataSet.read_minute_data(data_path=os.path.join(read_path,factor),
                                                 date_range=read_range,
                                                 ticker_list=alphaset.ticker_list).sortby('date',ascending= True),
                    dim = 'variable')
        ...
        def main_process(alpha: np.ndarray, ind_risk: np.ndarray, industry: np.ndarray, weight: np.ndarray, flag: np.ndarray, method: str = 'indmean', mode: str = 'mad', orth: bool = True) -> np.ndarray:
            # 用flag筛选ticker_list
        ...
        def xr_process(alpha: xr.DataArray, ind_risk: xr.DataArray, industry: xr.DataArray, weight: xr.DataArray, flag: xr.DataArray, method: str = 'indmean', mode: str = 'mad', orth: bool = True) -> xr.DataArray:
            alpha = xr.apply_ufunc(
                main_process, alpha,ind_risk,industry,weight,flag,
                ...
                kwargs = {'method':method,'mode':mode,'orth':orth}, # !必须以关键字的形式传进去
                vectorize = True,               
                dask="parallelized",
        ...
        read_range, remain_range = select_date(alphaset,resid_path = save_path)
        print('已经有的日期:%s'%read_range)
        print('需要加的日期:%s'%remain_range)
        if read_range != []:
            load_cache(alphaset, read_range, read_path = save_path)
            print('已有日期已经读取')
        if remain_range != []:
            # alpha的time是dataset的time的子集.
            alpha = alphaset.alpha.sel(date = remain_range).reindex(time = dataset.timelist).rename({'variable':'alpha'})
            ind_risk = xr.concat([dataset.industry_dummies.sel(date = remain_range),dataset.risk_factor.sel(date = remain_range)],dim = 'variable').squeeze(dim = 'time')
            ...
            alpha_resid = xr_process(alpha,ind_risk,industry,weight,flag,orth = orth,method = method,mode = mode).rename(alpha = 'variable').transpose('ticker','date','time','variable')
            alpha_resid = xr.where((dataset.price_ipo_st_flag.sel(date = remain_range).squeeze()==1) & (dataset.flag.sel(date = remain_range).squeeze()==0), 0, alpha_resid)
            alphaset.residual[self.fitter_name] = BaseDataSet.concat_on_dim(alphaset.residual[self.fitter_name], alpha_resid, dim = 'date').reindex(date=dataset.date_range)
            
        # 按因子存
        def to_dict(beta: xr.DataArray):
            newBeta = {}
            for var in beta['variable'].values:
                newBeta.update({var:beta.sel(variable = [var])})
            return newBeta

        BaseDataSet.save_DataArray(to_dict(alphaset.residual[self.fitter_name]), save_path, byDate=True)
        print('共耗时%f s'%(tm.time()-start_time))
    @staticmethod
    def alpha_autocorr(alpha: xr.DataArray, dataset: BaseDataSet, method: str='pearson', lag: int = 10) -> xr.DataArray:
        # NaN值不影响自相关的计算.
        ...
        alpha = alpha.reindex(time = dataset.timelist,fill_value=np.nan).where(dataset.flag.loc[:,:,:,'flag']==1)
    ...
    @staticmethod
    def turnover(alpha: xr.DataArray, dataset: BaseDataSet) ->xr.DataArray:
        # 每期的票不一样,该期存在,上一期不存在的票,上一期的因子值填为0
        alpha = alpha.reindex(time = dataset.flag['time'],fill_value=np.nan).where(dataset.flag.loc[:,:,:,'flag']==1,0).fillna(0)
        turnover = (np.abs(alpha.stack(datetime = ('date','time'))-alpha.stack(datetime = ('date','time')).shift(datetime = 1)).sum(dim='ticker')/np.abs(alpha.stack(datetime = ('date','time')).shift(datetime = 1)).sum(dim = 'ticker')).unstack()
        turnover = xr.where(turnover==np.inf,1,turnover)
        turnover = turnover.transpose('date','time','variable')
        return turnover

    @staticmethod
    def get_beta_t(alpha: xr.DataArray, dataset: BaseDataSet, var: List[str] = None, multi_factor: bool = True, add_ind_risk: bool = True) -> xr.DataArray:
        #! method 1
        ...
        def xr_ols(Return, alpha, ind_risk, weight, flag, multi_factor: bool = True, add_ind_risk: bool = True):
            if multi_factor:
                beta, t_value = xr.apply_ufunc(
                    ...
                    input_core_dims=[['ticker'],['ticker','alpha'],['ticker','ind_risk'],['ticker'],['ticker']],            # 每个参数只有一个条目的列表
                    output_core_dims=[['alpha'],['alpha']],             # 计算beta:'variable',计算resid:'ticker'
                    exclude_dims=set(('ticker',)),                      # 结果变量中要删除的变量
            ...
            beta = beta.rename(alpha = 'variable')
            t_value = t_value.rename(alpha = 'variable')
            return beta, t_value
        
        if var is not None:
            Return = dataset.y.sel(variable = [var]).fillna(0).rename({'variable':'y'})
        else:
            Return = dataset.y.fillna(0).rename({'variable':'y'})

        alpha = alpha.reindex(time = dataset.timelist).rename({'variable':'alpha'}).fillna(0)
        ind_risk = xr.concat([dataset.industry_dummies,dataset.risk_factor],dim = 'variable').squeeze(dim = 'time').rename(variable = 'ind_risk')
        weight = dataset.capital.squeeze(dim = ['variable','time'])
        flag = dataset.flag.squeeze(dim = 'variable')
        beta, t_value = xr_ols(Return, alpha, ind_risk, weight, flag, multi_factor = multi_factor, add_ind_risk = add_ind_risk)

        #! method 2 
        # beta = xr.DataArray(coords = list([alpha['date'],alpha['time'],alpha['variable']]),dims=['date','time','variable'])
        # t_value = xr.DataArray(coords = list([alpha['date'],alpha['time'],alpha['variable']]),dims=['date','time','variable'])
        # for idate in alpha['date'].values:
        #     risk_factor = pd.DataFrame(np.squeeze(dataset.risk_factor.loc[:,idate,145900,:].values),index = dataset.risk_factor['ticker'], columns = dataset.risk_factor['variable'])
        #     industry_dummies = pd.DataFrame(np.squeeze(dataset.industry_dummies.loc[:,idate,145900,:].values),index = dataset.industry_dummies['ticker'], columns = dataset.industry_dummies['variable'])
        #     ind_risk = pd.concat([industry_dummies, risk_factor],axis = 1)
        #     for itime in alpha['time'].values:
        #         index_itime = pd.Series(dataset.ticker_list).loc[dataset.flag.loc[:,idate,itime,'flag'].values==1].tolist()
        #         alpha_factor = pd.DataFrame(np.squeeze(alpha.loc[index_itime,idate,itime,:].values),index = index_itime, columns = alpha['variable'])
        #         ind_risk_itime = ind_risk.loc[index_itime,:]
        #         y = dataset.Return.loc[index_itime,idate,itime,'AccumulatedReturn'].fillna(0)
        #         weight = (1 / np.squeeze(np.sqrt(dataset.capital.loc[index_itime,idate,:,:]))).fillna(0).values
        #         if multi_factor:
        #             X = pd.concat([alpha_factor, ind_risk_itime],axis = 1).fillna(0)
        #             fit = sm.WLS(y.astype(float),X.astype(float),weights=weight).fit()
        #             beta.loc[idate,itime,:] = fit.params[alpha['variable'].values]
        #             t_value.loc[idate,itime,:] = fit.tvalues[alpha['variable'].values]
        #         else:
        #             for ialpha in alpha['variable'].values:
        #                 X = pd.concat([alpha_factor.loc[:,ialpha], ind_risk_itime],axis = 1).fillna(0)
        #                 fit = sm.WLS(y.astype(float),X.astype(float),weights=weight).fit()
        #                 beta.loc[idate,itime,ialpha] = fit.params[ialpha]
        #                 t_value.loc[idate,itime,ialpha] = fit.tvalues[ialpha]
        return beta,t_value

    @staticmethod
    def IC(alpha: xr.DataArray, dataset: BaseDataSet, beta: xr.DataArray, method: str='pearson') -> Tuple[xr.DataArray]:  
        ...
        def xr_IC(alpha, Return, beta, method: str = 'pearson'):
            ...
            return IC.transpose('date','time','variable','y'), IC_long.transpose('date','time','variable','y'), IC_short.transpose('date','time','variable','y')
        
        alpha = alpha.reindex(time = dataset.timelist,fill_value=np.nan).where(dataset.flag.loc[:,:,:,'flag']==1)
        # !有点问题Return:
        Return = dataset.y.where(dataset.flag.squeeze()==1).fillna(0).rename({'variable':'y'})
        IC, IC_long, IC_short = xr_IC(alpha, Return, beta, method = method)
        
        #! method 2 
        # *如果不需要参数method,可以更简便地如下实现:
        # !由于服务器上xarray的版本太低了(0.14.0),还没有xr.corr这个函数,因此只能用else后的代码算
        # alpha_return = (alpha*beta).where(dataset.flag.loc[:,:,:,'flag']==1,np.nan)
        # IC = xr.corr(alpha,Return,dim = 'ticker')
        # IC_long  = xr.corr(alpha_return.where(alpha_return>alpha_return.quantile(2/3,dim = 'ticker')),dataset.Return.loc[:,:,:,'AccumulatedReturn'].where(alpha_return>alpha_return.quantile(2/3,dim = 'ticker')),dim = 'ticker')
        # IC_short = xr.corr(alpha_return.where(alpha_return<alpha_return.quantile(1/3,dim = 'ticker')),dataset.Return.loc[:,:,:,'AccumulatedReturn'].where(alpha_return<alpha_return.quantile(1/3,dim = 'ticker')),dim = 'ticker')

        #! method 3 
        # *如果需要参数method,则需要如下方式实现:
        # IC = xr.DataArray(coords = list([alpha['date'], alpha['time'], alpha['variable']]), dims = ['date','time','variable'])
        # IC_long = xr.DataArray(coords = list([alpha['date'], alpha['time'], alpha['variable']]), dims = ['date','time','variable'])
        # IC_short = xr.DataArray(coords = list([alpha['date'], alpha['time'], alpha['variable']]), dims = ['date','time','variable'])
        # for idate in alpha['date'].values:
        #     return_date = pd.DataFrame(np.squeeze(dataset.Return.loc[:,idate,:,'AccumulatedReturn'].values),index = dataset.Return['ticker'], columns = dataset.Return['time'])
        #     for ialpha in alpha['variable'].values:
        #         alpha_date = pd.DataFrame(np.squeeze(alpha.loc[:,idate,:,ialpha].values),index = alpha['ticker'], columns = alpha['time'])
        #         alpha_return_date = pd.DataFrame(np.squeeze(alpha_return.loc[:,idate,:,ialpha].values),index = alpha_return['ticker'], columns = alpha_return['time'])               
        #         IC.loc[idate,:,ialpha] = return_date.corrwith(alpha_date, axis=0, method = method)
        #         IC_long.loc [idate,:,ialpha] = return_date.where(alpha_return_date>alpha_return_date.quantile(2/3)).corrwith(alpha_return_date.where(alpha_return_date>alpha_return_date.quantile(2/3)), axis=0, method = method)
        #         IC_short.loc[idate,:,ialpha] = return_date.where(alpha_return_date<alpha_return_date.quantile(1/3)).corrwith(alpha_return_date.where(alpha_return_date<alpha_return_date.quantile(1/3)), axis=0, method = method)
        return IC, IC_long, IC_short

    @staticmethod
    def single_sort(alpha: xr.DataArray, dataset: BaseDataSet, N: int = 5, neutralizing: bool=True, cost_rate: int = 0.0008) -> dict:
        alpha = alpha.reindex({'time':dataset.timelist}).fillna(0).where(dataset.flag.loc[:,:,:,'flag']==1,np.nan) #有可能因子值在某几个时间点没有# alpha为np.nan都是被剔除掉的票
        ...
        def xr_rank(arr: xr.DataArray) -> xr.DataArray:
            ...
            for factor in arr['variable'].values:
                arr_rank.loc[:,:,factor] = pd.DataFrame(arr.loc[:,:,factor].values,index = arr['ticker'],columns = arr['datetime']).rank(axis = 0,method='first',pct = True).values    
            return arr_rank.unstack().transpose('ticker','date','time','variable')        
        def xr_str(arr0,arr1):
            def add_str(obj0,obj1):
                return str(obj0)+str(obj1)
            out = xr.apply_ufunc(
                add_str,arr0,arr1,input_core_dims=[[],[]],output_core_dims=[[]],vectorize = True,dask="parallelized"
                )
            return out
        
        if neutralizing:     
            capital =  dataset.capital.reindex({'time':dataset.timelist},method = 'bfill').where(dataset.flag.loc[:,:,:,'flag']==1,np.nan)
            industry= dataset.industry.reindex({'time':dataset.timelist},method = 'bfill').where(dataset.flag.loc[:,:,:,'flag']==1,np.nan).fillna(0).astype(int)
            labels_cap = xr.full_like(capital, np.nan)
            labels_cap = xr.where((capital>=capital.quantile(0,  dim = 'ticker'))&(capital< capital.quantile(1/3,dim = 'ticker')),'small', labels_cap)
            labels_cap = xr.where((capital>=capital.quantile(1/3,dim = 'ticker'))&(capital< capital.quantile(2/3,dim = 'ticker')),'medium',labels_cap)
            labels_cap = xr.where((capital>=capital.quantile(2/3,dim = 'ticker'))&(capital<=capital.quantile(1  ,dim = 'ticker')),'big',   labels_cap)
            labels =  xr_str(labels_cap.squeeze(dim = 'variable'),industry.squeeze(dim = 'variable')).expand_dims(dim = {'variable':alpha['variable'].values}).transpose('ticker','date','time','variable') # 四个维度，ticker,date,time,variable 
            ...
            labels = labels.where(dataset.flag.loc[:,:,:,'flag']==1,np.nan) # 剔除未标flag的票
            label_names = ['n']
        iportfolio_return = xr.zeros_like(alpha).expand_dims(dim =  {'group':groups}).stack(datetime = ('date','time')).transpose('ticker','datetime','variable','group').copy()
        for ilabel in label_names:
            ialpha_pct = xr_rank(xr.where(labels == ilabel,alpha,np.nan))#da.rank(dim = 'ticker', pct=True) # ialpha_pct:ticker,date,time,variable # xr.DataArray.rank函数会忽略NaN,结果返回为NaN
            for i in range(N):
                iflag = xr.where((ialpha_pct>(i/N))&(ialpha_pct<=((i+1)/N)),1,np.nan).fillna(0).stack(datetime = ('date','time'))
                # average_return_30 = (np.exp(np.log(1 + dataset.Return.loc[:,:,:,'MinuteReturn'].fillna(0).stack(datetime=('date', 'time'))).rolling(datetime=period).mean()) - 1).shift(datetime=-(period + dataset.lag_period)).unstack()
                iportfolio_return.loc[:,:,:,'portfolio_'+str(i+1)] = iportfolio_return.loc[:,:,:,'portfolio_'+str(i+1)] + dataset.Return.loc[:,:,:,'MinuteReturn'].stack(datetime = ('date','time')).fillna(0) * iflag
        iportfolio_return.loc[:,:,:,'portfolio_long_short'] = iportfolio_return.loc[:,:,:,'portfolio_'+str(N)] - iportfolio_return.loc[:,:,:,'portfolio_1']
        turnover = iportfolio_return.where(iportfolio_return>0).notnull().astype(int)
        turnover = turnover / turnover.sum(dim = 'ticker')
        turnover_rate = np.abs(turnover-turnover.shift(datetime=1)).sum(dim = 'ticker')
        turnover_rate.loc[:,:,'portfolio_long_short'] = turnover_rate.loc[:,:,'portfolio_'+str(N)] + turnover_rate.loc[:,:,'portfolio_1']
        turnover_rate = turnover_rate.unstack().transpose('date','time','variable','group')  
        portfolio_return = xr.where(iportfolio_return==0,np.nan,iportfolio_return).mean(dim = 'ticker').unstack().transpose('date','time','variable','group').fillna(0)
        portfolio_return_cost = portfolio_return - turnover_rate*cost_rate
        
        return portfolio_return,portfolio_return_cost,turnover_rate

    @staticmethod
    def plot(fit_data, save_path, width: str = '1500px', height: str = '800px'):   
        data = fit_data
        ...
        def set_global_opts(chart: Line,title: str):
            chart = chart.set_global_opts(
                ...
                yaxis_opts=opts.AxisOpts(min_='dataMin', splitline_opts=opts.SplitLineOpts(is_show=True))).set_series_opts(
                label_opts=opts.LabelOpts(is_show=False))
            return chart

        Xaxis = [str(idate) for idate in data['sort']['date'].values]
        Xaxis.insert(0,'preDay')
        Xaxis1 = [str(lag) for lag in data['alpha_autocorr']['lag'].values] # !自相关系数 滞后阶数从0开始
        newDate = np.r_[0,data['sort']['date'].values]
        ...
        for ifactor in data['beta']['variable'].values:
            ...
            sort_neutr      = Line(init_opts=opts.InitOpts(width=width, height=height)).add_xaxis(Xaxis)
            sort_cost       = Line(init_opts=opts.InitOpts(width=width, height=height)).add_xaxis(Xaxis)
            sort_neutr_cost = Line(init_opts=opts.InitOpts(width=width, height=height)).add_xaxis(Xaxis)
            for y in data['IC']['y'].values:
                exec('IC_{} = Line(init_opts=opts.InitOpts(width=width, height=height)).add_xaxis(Xaxis)'.format(y))
            beta            = Line(init_opts=opts.InitOpts(width=width, height=height)).add_xaxis(Xaxis)
            turnover        = Line(init_opts=opts.InitOpts(width=width, height=height)).add_xaxis(Xaxis)
            autocorr        = Line(init_opts=opts.InitOpts(width=width, height=height)).add_xaxis(Xaxis1) 
            for igroup in data['sort']['group'].values:
                sort            = sort.add_yaxis(igroup,(data['sort'].sel(variable = ifactor,group = igroup).fillna(0)+1).prod(dim = 'time').cumprod(dim = 'date').values.tolist())
                sort_neutr      = sort_neutr.add_yaxis(igroup,(data['sort_neutr'].sel(variable = ifactor,group = igroup).fillna(0)+1).prod(dim = 'time').cumprod(dim = 'date').values.tolist())
                sort_cost       = sort_cost.add_yaxis(igroup,(data['sort_cost'].sel(variable = ifactor,group = igroup).fillna(0)+1).prod(dim = 'time').cumprod(dim = 'date').values.tolist())
                sort_neutr_cost = sort_neutr_cost.add_yaxis(igroup,(data['sort_neutr_cost'].sel(variable = ifactor,group = igroup).fillna(0)+1).prod(dim = 'time').cumprod(dim = 'date').values.tolist())
            
            ICTab = Tab()
            for y in data['beta']['y'].values:
                beta = beta.add_yaxis(y,(data['beta'].sel(variable = ifactor,y = y).fillna(0)+1).prod(dim = 'time').cumprod(dim = 'date').values.tolist())
                names['IC_'+y] = (names['IC_'+y].add_yaxis('IC',data['IC'].sel(variable = ifactor,y = y).mean(dim = 'time').values.tolist())
                                                .add_yaxis('IC_long',data['IC_long'].sel(variable = ifactor, y = y).mean(dim = 'time').values.tolist())
                                                .add_yaxis('IC_short',data['IC_short'].sel(variable = ifactor, y = y).mean(dim = 'time').values.tolist()))
                names['IC_'+y] = set_global_opts(chart=names['IC_'+y],title=ifactor +'IC')
                ICTab = ICTab.add(names['IC_'+y],'IC_'+y)
                
                
            turnover = turnover.add_yaxis('turnover',data['turnover'].sel(variable = ifactor).mean(dim = 'time').values.tolist())
            autocorr = autocorr.add_yaxis('autocorr',data['alpha_autocorr'].sel(variable = ifactor).mean(dim = ['date','time']).values.tolist())
            sort            = set_global_opts(chart=sort,           title=ifactor +'portfolio')
            sort_neutr      = set_global_opts(chart=sort_neutr,     title=ifactor +'portfolio_neutralizating')
            sort_cost       = set_global_opts(chart=sort_cost,      title=ifactor +'portfolio_cost')
            sort_neutr_cost = set_global_opts(chart=sort_neutr_cost,title=ifactor +'portfolio_cost_neutralizating')
            beta            = set_global_opts(chart=beta,           title=ifactor +'Beta')
            turnover        = set_global_opts(chart=turnover,       title=ifactor +'turnover')
            autocorr        = set_global_opts(chart=autocorr,       title=ifactor +'autocorr')

            sortTab = (Tab().add(sort,'portfoilo').add(sort_neutr,'portfolio_neutralizating')
                            .add(sort_cost,'portfolio_cost').add(sort_neutr_cost,'portfolio_cost_neutralizating'))
            sortTab.render(os.path.join(save_path,f'{ifactor}_sort.html'))
            
            ICTab.render(os.path.join(save_path,f'{ifactor}_IC.html'))
            page = Page(layout=Page.SimplePageLayout) 
            page.add(beta, turnover, autocorr)
            page.render(os.path.join(save_path,f'{ifactor}_beta_turnover_autocorr.html'))
            print(f'{ifactor} Garph saved')

    def fit(self, alphaset: BaseAlphaSet, 
            dataset: BaseDataSet,
            var: List[str] = None,
            save_path: str = None,
            fit_result: BaseFitResult = None,
            multi_factor: bool = True,
            add_ind_risk: bool = True, 
            BetaOnly: bool = True,
            byDate: bool = True,
            isPlot: bool = False,) -> 'LinearRegressionResult':
        ...
        if fit_result is None:
            fit_result = LinearRegressionResult(self)
        ...
        beta, t_values = LinearRegressionFitter.get_beta_t(alphaset.residual[self.fitter_name],dataset,var = var, multi_factor=multi_factor,add_ind_risk = add_ind_risk)
        fit_result.beta, fit_result.t_values = concat_on_dim(fit_result.beta, beta, dim = 'date'), concat_on_dim(fit_result.t_values,t_values,dim = 'date')
        print('beta,tvalues计算完成!')
        ...
        # 把所有的结果弄成一个字典的形式
        BaseDataSet.save_DataArray(to_dict(beta), os.path.join(save_path,'beta'), byDate = byDate)
        BaseDataSet.save_DataArray(dict(tvalues = t_values), save_path, byDate = False)
        print('beta,tvalues已保存在%s!'%save_path)
        fit_data = dict(beta = beta)
        if not BetaOnly:
            IC, IC_long, IC_short = LinearRegressionFitter.IC(alphaset.residual[self.fitter_name],dataset,fit_result.beta)
            print('IC计算完成!')
            alpha_autocorr = LinearRegressionFitter.alpha_autocorr(alphaset.residual[self.fitter_name],dataset)
            print('alpha_autocorr计算完成!')
            turnover = LinearRegressionFitter.turnover(alphaset.residual[self.fitter_name],dataset)
            print('turnover计算完成!')
            sort, sort_cost, _ = LinearRegressionFitter.single_sort(alphaset.residual[self.fitter_name],dataset,neutralizing=True, cost_rate=0.0008)
            sort_neutr, sort_neutr_cost,_ = LinearRegressionFitter.single_sort(alphaset.residual[self.fitter_name],dataset,neutralizing=False, cost_rate=0.0008)
            print('分组计算完成!')
            # 做成一个字典,画图以及导出
            fit_data.update(dict(sort=sort, sort_neutr=sort_neutr, sort_cost=sort_cost, sort_neutr_cost=sort_neutr_cost,
                                 IC=IC, IC_long=IC_long, IC_short=IC_short, alpha_autocorr = alpha_autocorr, turnover=turnover))
            fit_result.IC, fit_result.IC_long, fit_result.IC_short = concat_on_dim(fit_result.IC, IC, dim = 'date'), concat_on_dim(fit_result.IC_long,IC_long,dim = 'date'), concat_on_dim(fit_result.IC_short,IC_short,dim = 'date')
            fit_result.alpha_autocorr, fit_result.turnover, fit_result.sort = concat_on_dim(fit_result.alpha_autocorr, alpha_autocorr, dim = 'date'), concat_on_dim(fit_result.turnover,turnover,dim = 'date'), concat_on_dim(fit_result.sort,sort,dim = 'date')
            BaseDataSet.save_DataArray(fit_data,save_path, byDate = False)
            print('IC,alpha_autocorr,turnover,分组结果已经保存!')
        if isPlot:
            LinearRegressionFitter.plot(fit_data,os.path.join(save_path,'single_factor_result'))
        print('共耗时%f s'%(tm.time()-start_time))
        return fit_result

class LinearRegressionResult(BaseFitResult):
    def __init__(self,fitter: LinearRegressionFitter):
    ...
    def calculate_attr(self):
        ...
        self.score.loc[:,'factor_return_IR1']  = (
                                                 ...
                                                 pd.Series(self.beta[:,int(len(self.beta['time'])*1/2):,:].std( dim=['date','time'],ddof=1)*np.sqrt(240),index = self.beta['variable'])
                                                 )
        ...
        self.score.loc[:,'factor_return_IR3']  = (
                                                 ...
                                                 pd.Series(self.beta[:,int(len(self.beta['time'])*1/4):,:].std( dim=['date','time'],ddof=1)*np.sqrt(240),index = self.beta['variable'])
                                                 )
        ...
        self.score.loc[:,'t2_ratio_positive']  = pd.Series((self.t_values>2).sum(dim=['date','time'])/(self.t_values>0).sum(dim=['date','time']))
        self.score.loc[:,'IC_mean']            = pd.Series(self.IC.mean(dim=['date','time']),index = self.IC['variable'])
        self.score.loc[:,'ICIR']               = self.score.loc[:,'IC_mean']/pd.Series(self.IC.std(dim=['date','time'],ddof=1)*np.sqrt(240),index = self.IC['variable'])
    ...
    def decorr(self, score_name: str, corr_thres: List[int], get_detail: bool = False) -> pd.DataFrame:
        ...
        def _corr(df1:pd.DataFrame, df2:pd.DataFrame) -> pd.DataFrame:
            ...
            corr = (
                (df1.T.dot(df2) - (df1_sum * df2_sum) / n_sample) / 
                np.sqrt(
                    ((df1 ** 2).T.dot(df2_flag) - (df1_sum ** 2) / n_sample) * 
                    ((df2 ** 2).T.dot(df1_flag).T - (df2_sum ** 2) / n_sample)
                )
            )
    ...
    def predict_beta(self, window, beta: xr.DataArray = None, date_range: List[str]= None, beta_path: str = None, isSave: bool = True, save_path: str = None):
        # 滚动三十天计算日度的beta
        if save_path is None:
            if ('fit_result_path' in setting.keys()) and (setting['fit_result_path'] is not None):
                save_path = setting['fit_result_path']
            else:
                save_path = os.path.join(setting['dst_dir'],'fit_result','predicted_beta')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
            ...
            if self.beta is not None:
                ...
                if beta_path is None:
                    if ('beta_path' in setting.keys()) and (setting['beta_path'] is not None):
                        ...
                        beta_path = os.path.join(setting['dst_dir'],'fit_result','beta')
                beta = None
        ...
        if isSave:
            def to_dict(arr: xr.DataArray):
                newArr = {}
                for var in arr['variable'].values:
                    newArr.update({var:arr.sel(variable = [var])})
                return newArr
            BaseDataSet.save_DataArray(to_dict(self.predicted_beta), save_path, byDate=True,separateFolder=True)
        
    def predict_return(self, dataset: BaseDataSet, alphaset: BaseAlphaSet, predicted_beta: xr.DataArray = None, save_path: str = None, predicted_beta_path: str = None):
        if save_path is None:
            if ('fit_result_path' in setting.keys()) and (setting['fit_result_path'] is not None):
                save_path = setting['fit_result_path']
            else:
            ...
            if self.predicted_beta is not None:
                ...
                if predicted_beta_path is None:
                    if ('predicted_beta_path' in setting.keys()) and (setting['predicted_beta_path'] is not None):
                        ...
                        predicted_beta_path = os.path.join(setting['dst_dir'],'fit_result','predicted_beta')
        ...
        temp1 = (alphaset.residual[self.fitter_name].sel(variable='mainS1')*predicted_beta.sel(variable='mainS1',y='high')).transpose('ticker','date','time')
        temp2 = (alphaset.residual[self.fitter_name].sel(variable='nonmainS1')*predicted_beta.sel(variable='nonmainS1',y='low')).transpose('ticker','date','time')
        temp = xr.concat([temp1,temp2], dim='variable')
        temp['variable'] = ['mainS1','nonmainS1']
        temp = dataset.datetime_lag(temp,lag=setting['lag'])
        pred_return = xr.where(temp.sel(variable = 'mainS1')>=0, temp.sel(variable = 'mainS1'), np.nan)
        pred_return = xr.where(temp.sel(variable = 'nonmainS1')<=0, temp.sel(variable = 'nonmainS1'), pred_return)
        self.predicted_return = xr.where((temp.sel(variable = 'mainS1')>0) & (temp.sel(variable = 'nonmainS1')<0),temp.sel(variable = 'mainS1')+temp.sel(variable = 'nonmainS1'), pred_return).expand_dims('variable').transpose('ticker','date','time','variable')
    ...
    def getWeight(self, dataset: BaseDataSet,
        ...
        def process_weight(Weight, TradableWeight, date, time):
            ...
            TradableWeight.loc[:, date, time, 'SellableWeight'] = Return.loc[:, date, time, 'MinuteReturn'] * TradableWeight.loc[:, date, time, 'SellableWeight'] / ScaleWeight
                ...
                for i in range(len(indlist)):
                    ...
                    if  ((diff>0) & (remain>0)): #行业i仍有可加的仓位    且          总仓位仍有可加的空间
                        ...
                        if not itoSell.empty:
                            ...
                            if not ticker.empty:
                                Weight.loc[ticker[0], date, time, 'Weight'] -= (itoSell.loc[ticker[0],'Weight'] + itoSell.loc[ticker[0],'AccumWeight'])
        ...
        def Swap(Weight, TradableWeight, date, time):
            ...
            if not iPR.empty:
                for i in range(indNum):
                    ...
                    SellWeight = iTradableWeight['SellableWeight'][iTradableWeight['SellableWeight'] > 0].rename('Weight')
                    BuyWeight = iTradableWeight['BuyableWeight'][iTradableWeight['BuyableWeight'] > 0].rename('Weight')
                    Sell = pd.concat([SellWeight, iPR], axis=1, join='inner').dropna(how='any').reset_index().rename(columns = {'PR':'PR-','ticker':'ticker-'}).sort_values(by='PR-',ascending = True)
                    Buy = pd.concat([BuyWeight, iPR], axis=1, join='inner').dropna(how='any').reset_index().rename(columns = {'PR':'PR+','ticker':'ticker+'}).sort_values(by='PR+',ascending = False)
            ...
            if not Diff.empty:
                ...
                if not Diff.empty:
                    ...
                    Weight.loc[Change['ticker+'].unique(),date,time,'Weight'] += Change.groupby('ticker+',sort=False)['weight_diff'].sum()[Change['ticker+'].unique()]
                    Weight.loc[Change['ticker-'].unique(),date,time,'Weight'] -= Change.groupby('ticker-',sort=False)['weight_diff'].sum()[Change['ticker-'].unique()]
                    # TradableWeight.loc[Change['ticker+'].unique(),date,time,'BuyableWeight'] -= Change.groupby('ticker+')['weight_diff'].sum().values
                    TradableWeight.loc[Change['ticker-'].unique(),date,time,'SellableWeight']-= Change.groupby('ticker-',sort=False)['weight_diff'].sum()[Change['ticker-'].unique()]
                    TradableWeight.loc[:, date, time, 'BuyableWeight'] = np.maximum(MaxStockPosition - Weight.loc[:, date, time, 'Weight'], 0)
            ...
            if self.predicted_return is not None:
                ...
                if predicted_return_path is None:
                    if ('predicted_return_path' in setting.keys()) and (setting['predicted_return_path'] is not None):
                        ...
                        predicted_return_path = os.path.join(setting['dst_dir'],'fit_result','predicted_return')
                PredictedReturn = BaseDataSet.read_minute_data(data_path=os.path.join(predicted_return_path),date_range=dataset.date_range,ticker_list=dataset.ticker_list)
        ...
        turnover: xr.DataArray = dataset.creat_dataarray(var = ['TotalTO','SwapTO','MatchTO']).reindex(date=datelist_delay).sum(dim='ticker').fillna(0)
        ...
        for date in datelist_delay:
            ...
            for time in dataset.timelist:
                ...
                weight_scaled = Weight.loc[:, date, time, 'Weight']
                if Relative:
                    ...
                    turnover.loc[date,time,'SwapTO'] = np.abs(Weight.loc[:,date,time,'Weight']-weight_scaled).sum(dim='ticker')-turnover.loc[date,time,'MatchTO']
                else:
                ...
                pre_weight = Weight.loc[:, date, time, 'Weight']
    ...
    def getPortReturn(self, dataset: BaseDataSet, 
        ...
        def xr_scaleweight(Return: xr.DataArray, weight: xr.DataArray, isTimeScale: bool = True):
            if isTimeScale:
                scale_weight, weight = xr.apply_ufunc(
                    timescale, Return, weight,input_core_dims=[['ticker'],['ticker']],output_core_dims=[['ticker'],['ticker']],            
                    vectorize = True,dask="parallelized",output_dtypes=[weight.dtype, weight.dtype])
            else:
                scale_weight, weight = xr.apply_ufunc(
                    datescale, Return, weight,input_core_dims=[['ticker','time'],['ticker','time']],output_core_dims=[['ticker','time'],['ticker','time']],            # 计算beta:'variable',计算resid:'ticker'
                    vectorize = True,dask="parallelized",output_dtypes=[weight.dtype, weight.dtype])
            return scale_weight.fillna(0), weight.fillna(0)
        
        def calcStatistics(weight: xr.DataArray, scale_weight: xr.DataArray, BMweight: xr.DataArray, Return: xr.DataArray, save_path: str, costRate: int = 0.0015):
            ...
            # Time
            turnoverTime = turnover.mean(dim = 'date')
            # 折线图
            ...
            plot_data = {'totalWeightDate': totalWeightDate, 'turnoverDate': turnoverDate,
                        'portReturnDate': portReturnDate, 'portReturnNetDate': portReturnNetDate, 'BMReturnDate': BMReturnDate,
                        'portReturn_BMDate': portReturn_BMDate, 'portReturnNet_BMDate': portReturnNet_BMDate,
                        'turnoverTime': turnoverTime,
                        'portReturnTime': portReturnTime, 'portReturnNetTime': portReturnNetTime, 'BMReturnTime': BMReturnTime,
                        'portReturn_BMTime': portReturn_BMTime, 'portReturnNet_BMTime': portReturnNet_BMTime,                
                        }
            ...
            stats = pd.concat([annual_return.rename('年化收益率'),tracking_error.rename('跟踪误差(年化波动率)'),sharp_ratio.rename('夏普比率'),
                            maximum_drawdown.rename('最大回撤'),meanTurnover.rename('平均换手率')],axis=1).T
            with pd.ExcelWriter(os.path.join(save_path,'stats.xlsx')) as writer:
                ...
                pd.Series(turnoverDate.values,index = turnoverDate['date'].values).to_excel(writer,sheet_name='Date_turnover')
                pd.Series(turnoverTime.values,index = turnoverTime['time'].values).to_excel(writer,sheet_name='Time_turnover')
            print('Stats_excel Saved! Port_net_value_excel Saved!')
            return portReturn, turnover, portReturnNet, plot_data, stats
        
        def plot_netValue(plot_data, save_path: str):
            
            def BarDate(lastweightDate, turnoverDate, portReturnDate, portReturnNetDate, BMReturnDate, portReturn_BMDate, portReturnNet_BMDate) -> Bar:
                date_axis            = [str(idate) for idate in list(portReturnDate['date'].values)]
                lastweightDate       = list(lastweightDate.values)
                turnoverDate         = list(turnoverDate.values)
                ...
                bar = (Bar(init_opts=opts.InitOpts(width='1500px', height='900px'))
                        ...
                        .extend_axis(yaxis=opts.AxisOpts(type_="value",name="收益率",max_='dataMax',min_='dataMin',position="left",axisline_opts=opts.AxisLineOpts(
                                    linestyle_opts=opts.LineStyleOpts(color="#215EFF")),axislabel_opts=opts.LabelOpts(formatter="{value}"),
                                    splitline_opts=opts.SplitLineOpts(is_show=True, linestyle_opts=opts.LineStyleOpts(opacity=1)),))
                        .set_global_opts(yaxis_opts=opts.AxisOpts(name="换手率",max_='dataMax',min_='dataMin',position="right",axisline_opts=opts.AxisLineOpts(linestyle_opts=opts.LineStyleOpts(color="#d14a61")),),
                                        title_opts=opts.TitleOpts(title="组合收益及换手率--date"),tooltip_opts=opts.TooltipOpts(trigger="axis", axis_pointer_type="cross"),
                                        xaxis_opts=opts.AxisOpts(min_='dataMin'),legend_opts=opts.LegendOpts(type_="scroll", pos_left="right",orient="vertical")))
                line = (Line()
                        .add_xaxis(date_axis)
                        .add_yaxis("return",         portReturnDate,       yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#5793f3'))     
                        .add_yaxis("return-cost",    portReturnNetDate,    yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#A86C0C'))
                        .add_yaxis("bm",             BMReturnDate,         yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#d14a61'))
                        .add_yaxis("return-bm",      portReturn_BMDate,    yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#FF9A5B'))
                        .add_yaxis("return-bm-cost", portReturnNet_BMDate, yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#FF9A5B')))
                bar.overlap(line)
                return bar

            def BarTime(turnoverTime, portReturnTime, portReturnNetTime, BMReturnTime, portReturn_BMTime, portReturnNet_BMTime) -> Bar:
                time_axis            = [str(itime) for itime in list(portReturnTime['time'].values)]
                turnoverTime         = list(turnoverTime.values)
                ...
                bar = (Bar(init_opts=opts.InitOpts(width='1500px', height='900px'))
                        ...
                        .extend_axis(yaxis=opts.AxisOpts(type_="value",name="收益率",max_='dataMax',min_='dataMin',position="left",axisline_opts=opts.AxisLineOpts(
                                    linestyle_opts=opts.LineStyleOpts(color="#215EFF")),axislabel_opts=opts.LabelOpts(formatter="{value}"),
                                    splitline_opts=opts.SplitLineOpts(is_show=True, linestyle_opts=opts.LineStyleOpts(opacity=1)),))
                        .set_global_opts(yaxis_opts=opts.AxisOpts(name="换手率",max_='dataMax',min_='dataMin',position="right",axisline_opts=opts.AxisLineOpts(
                                        linestyle_opts=opts.LineStyleOpts(color="#d14a61")),),datazoom_opts=opts.DataZoomOpts(),
                                        title_opts=opts.TitleOpts(title="组合收益及换手率--date"),tooltip_opts=opts.TooltipOpts(trigger="axis", axis_pointer_type="cross"),
                                        xaxis_opts=opts.AxisOpts(min_='dataMin'),legend_opts=opts.LegendOpts(type_="scroll", pos_left="right",orient="vertical")))
                line = (Line()
                        .add_xaxis(time_axis)
                        .add_yaxis("return",         portReturnTime,       yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#5793f3'))     
                        .add_yaxis("return-cost",    portReturnNetTime,    yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#A86C0C'))
                        .add_yaxis("bm",             BMReturnTime,         yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#d14a61'))
                        .add_yaxis("return-bm",      portReturn_BMTime,    yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#FF9A5B'))
                        .add_yaxis("return-bm-cost", portReturnNet_BMTime, yaxis_index=1, label_opts = opts.LabelOpts(is_show=False),itemstyle_opts=opts.ItemStyleOpts(color='#FF9A5B')))
                bar.overlap(line)
                return bar

            (Tab().add(BarDate(plot_data['totalWeightDate'],plot_data['turnoverDate'],
                               plot_data['portReturnDate'],plot_data['portReturnNetDate'],plot_data['BMReturnDate'],
                               plot_data['portReturn_BMDate'],plot_data['portReturnNet_BMDate']), "Date")
                  .add(BarTime(plot_data['turnoverTime'],
                               plot_data['portReturnTime'],plot_data['portReturnNetTime'],plot_data['BMReturnTime'],
                               plot_data['portReturn_BMTime'],plot_data['portReturnNet_BMTime']), "Time")
                  .render(save_path))
            print("Net Value Graph saved")
                 
        if save_path is None:
            if 'portReturn_save_path' in setting.keys():
                if setting['portReturn_save_path'] != None:
                    save_path = setting['weight_save_path']
                else:
                    save_path = os.path.join(setting['dst_dir'],'fit_result','portReturn')
            else:
                    save_path = os.path.join(setting['dst_dir'],'fit_result','portReturn')        
        ...
        weight_date = dataset.date_range[Delay:]
            ...
            if self.weight is not None:
                ...
                if weight_path is None:
                    if ('weight_path' in setting.keys()) and (setting['weight_path'] is not None):
                        ...
                        weight_path = os.path.join(setting['dst_dir'],'fit_result','weight')
                weight = BaseDataSet.read_minute_data(weight_path, weight_date, dataset.ticker_list).fillna(0)
        ...
        weight_ = weight.stack(datetime=['date','time']).shift(datetime=1).unstack().fillna(0).transpose('ticker','date','time')
        datelist = weight['date'].values.tolist()
        BM = dataset.benchmark.squeeze(dim = 'variable').reindex(time=dataset.timelist, date=datelist).fillna(0)
        if Return is None:
            Return = dataset.Return.loc[:,:,:,'MinuteReturn'].reindex(date=datelist).fillna(0)+1
        scale_weight, weight_ = xr_scaleweight(Return, weight_, isTimeScale = True)
        weight_ = weight_.stack(datetime = ['date','time']).shift(datetime = -1).unstack().transpose('ticker','date','time')
        weight_[:,-1,-1] = weight[:,-1,-1]
        _, BMweight = xr_scaleweight(Return, BM, isTimeScale = False)
        # calcStatistics传入的weight没有滞后,scale滞后一期的,BMweight滞后一期
        portReturn, turnover, portReturnNet, plot_data, stats = calcStatistics(weight_, scale_weight, BMweight, Return, save_path, costRate)
        if issave:
    ...
    def calc_netValue(self,
        ...
        if save_path is None:
            if ('fit_result_path' in setting.keys()) and (setting['fit_result_path'] is not None):
                ...
                save_path = os.path.join(setting['dst_dir'], 'fit_result','net_value')
        ...
        if weight is None:
            if self.weight is not None:
                ...
                if ('weight_path' in setting.keys()) and (setting['weight_path'] is not None):
                    ...
                    weight_path = os.path.join(setting['fit_result_path'], 'weight')
                else:
                    weight_path = os.path.join(setting['dst_dir'], 'fit_result', 'weight')
    ...
    def plot_netValue(self,
                      plot_data: Dict[str, xr.DataArray] = None,
        ...
        if save_path is None:
            if ('fit_result_path' in setting.keys()) and (setting['fit_result_path'] is not None):
                ...
                save_path = os.path.join(setting['dst_dir'], 'fit_result', 'net_value_plot')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        ...
        if plot_data is not None:
            ...
            if load_path is None:
                if ('net_value_path' in setting.keys()) and (setting['net_value_path'] is not None):
                    ...
                    load_path = os.path.join(setting['fit_result'], 'net_value')
                else:
                    load_path = os.path.join(setting['dst_dir'], 'fit_result', 'net_value')
    ...
    def returnAttri(self, dataset: BaseDataSet, 
                    Return: xr.DataArray = None, 
                    weight: xr.DataArray = None, 
                    weight_path: str = None, 
                    save_path: str = None, 
        ...
        def return_adjust(Return: xr.DataArray,method = 'carino') -> xr.DataArray:
            ...
            df = Return.squeeze(dim = 'variable').stack(datetime = ('date','time')).transpose('datetime','variable').to_dataframe(name = 'values').pivot(index = 'datetime',columns = 'variable',values = 'values')
            ...
            df['adjust'] = c_t
            df[df.columns.difference(['portfolio_return','adjust'])] = df[df.columns.difference(['portfolio_return','adjust'])].multiply(df.loc[:,'adjust'],axis = 'index')
        ...
        def get_beta(dataset: BaseDataSet, Return: xr.DataArray) -> xr.DataArray:
            ...
            X = xr.concat([dataset.industry_dummies,dataset.risk_factor],dim = 'variable').squeeze(dim = 'time').where(dataset.flag.squeeze()==1).fillna(0)
            w = (1/np.sqrt(dataset.capital)).fillna(0).squeeze(dim = ['variable','time']).where(dataset.flag.squeeze()==1).fillna(0)
            beta = xr_ols(y,X,w)
            return beta

        def return_dec(dataset: BaseDataSet, Return: xr.DataArray, weight: xr.DataArray, beta: xr.DataArray, adjust:bool = False) -> Tuple[xr.DataArray]:
            portfolio_return = (weight.squeeze()*Return.squeeze()).sum(dim = 'ticker').expand_dims(dim = {'variable':'portfolio_return'})
            portfolio_factor = weight.squeeze(dim = 'variable')*xr.concat([dataset.industry_dummies,dataset.risk_factor],dim = 'variable').sum(dim = 'ticker')
        ...
        weight = weight.squeeze().fillna(0).stack(datetime = ('date','time')).shift(datetime = 1).unstack().reindex(ticker = dataset.ticker_list).transpose('ticker','date','time')

        if BM:
            weight = ((weight - dataset.benchmark.fillna(0).squeeze(dim = ['time','variable'])).squeeze(dim = 'variable').transpose('ticker','date','time'))
...
setting = {"start_date": 20210517, 
           "end_date": 20220421,
           "daily_lag": True,
           "timelist": None,
           "Return_path": None,
           "weight_path": None,
           "varlist": ['Return','benchmark'],
           #全部可加的varlist ['market','Return','transaction','index','risk_factor','industry_dummies','industry','capital','benchmark','flag','price_ipo_st_flag']
           "lag": 2, #滞后的分钟
           "BaseDataSet_path": None, #保存新算BaseDataSet
           "y_path": None,
           'predicted_return_path':None,
           "trade_calendar_path": "/usr/datashare/risk_factors_backup",
           "daily_data_path": "/usr/datashare/risk_factors_backup",
           "timelist_path": "/usr/intern/",
           "market_path": "/usr/datashare/level2/Market/1m_20220511/",
           "transaction_path": "/usr/datashare/level2/Transaction/1m/",
           "index_path": None,
           "risk_factor_path": "/usr/datashare/risk_factors_backup", 
           "alpha_path": "/usr/intern/Alpha/cashflow_factor0511/", 
           "dst_dir": "/usr/intern/BacktestData", #所有数据的默认存放和读取路径
           "univ_flag_file_name": "flag_return.csv", 
           "ipo_st_flag_file_name": "flag_ipo_st.csv",
           "timelist_file_name": "minute_timelist.csv",
           "market_fields": ["Close", "preDayClose"],
...
if __name__ == '__main__':
    # 一段短的时间
    # dataset = BaseDataSet(setting) #需要用到的基本的数据
    # # dataset.export_variables()
    # dataset.calc_y(lag = -2) #lag应该为负数, '提前'而不是'滞后'
    # # dataset.add_y()
    # alphaset = BaseAlphaSet(setting)
    # alphaset.add_alpha(alpha_dir=setting['alpha_path']) #读进来处理alpha
    ...
    # fit_result = LinearRegressionResult(fitter)
    # fit_result = fitter.fit(alphaset, dataset, fit_result=fit_result, multi_factor=False, add_ind_risk=True, BetaOnly=True, isPlot=False) #同时会保存结果
    # # single_sort运行有点慢,占用内存有点多.
    # fit_result.predict_beta(window=30)
    # fit_result.predict_return(dataset, alphaset)
    # fit_result.getWeight(dataset, Delay=30, SwapBenchmark=0.0030)
    # fit_result.getPortReturn(dataset, Delay=30)
    start_date: int = 20210517
    end_date: int = 20220421

    # 30天跑一次
    # 画图怎么画: 每次运行

    cycleDate = 30
    date_range = BaseDataSet.get_tradingday_range(setting['trade_calendar_path'],start_date=start_date,end_date=end_date)
    ...
    for i in range(cycleNum):
        setting['start_date'] = date_range[cycleDate * i]
        setting['end_date'] = date_range[min(cycleDate * i + cycleDate - 1, date_range.index(date_range[-1]))]

        dataset = BaseDataSet(setting) 
        dataset.calc_y(lag = -2) 
        
        alphaset = BaseAlphaSet(setting)
        alphaset.add_alpha(alpha_dir=setting['alpha_path']) 
        fitter.pre_processing(alphaset,dataset,orth=True) 
        fit_result = fitter.fit(alphaset, dataset, fit_result=fit_result, multi_factor=False, add_ind_risk=True, BetaOnly=True, isPlot=False) #同时会保存结果

    fit_result = LinearRegressionResult(fitter)
    fit_result.predict_beta(window=30)
    path__ = '/usr/intern/BacktestData/fit_result/weight/20220324.h5'
    weight__ = xr.open_dataarray(path__)
    pre_weight = weight__.sel(date=20220324, time=145900, variable='Weight')
    date_range_ = date_range[30:]
    cycleNum_ = np.ceil(len(date_range_) / cycleDate).astype(int)

    for i in range(cycleNum_):
        setting['start_date'] = date_range_[cycleDate * i]
        setting['end_date'] = date_range_[min(cycleDate * i + cycleDate - 1, date_range_.index(date_range_[-1]))]
        ...
        _,_,pre_weight = fit_result.getWeight(dataset, Delay=0, pre_weight=pre_weight, isReturn=True, SwapBenchmark=0.0030)
        
        fit_result.getPortReturn(dataset, Delay=30)
        fit_result.calc_netValue(dataset, Delay=0, isLoadTO=False)

    fit_result = LinearRegressionResult(fitter)
    fit_result.plot_netValue(Delay=None)
